...
document.getElementById('runBtn').addEventListener('click', ()=>{
  ...
  drawTrajectory(miss, finalMiss);
  drawBPlane(miss, finalMiss);
});

function drawTrajectory(miss, finalMiss){
  ctxT.clearRect(0,0,traj.width,traj.height);
  const W = traj.width, H = traj.height;
  const scale = W/(miss*2+100000);
  const planetX = W-100;
  const midY = H/2;
  ctxT.fillStyle='#0ea5a4';
  ctxT.beginPath();
  ctxT.arc(planetX,midY,100,0,Math.PI*2);
  ctxT.fill();
  ctxT.strokeStyle='#f59e0b';
  ctxT.beginPath();
  ctxT.moveTo(50, midY - miss*scale);
  ctxT.lineTo(planetX, midY - finalMiss*scale);
  ctxT.stroke();
}
...
function drawBPlane(miss, finalMiss){
  ...
  ctxB.beginPath();
  ctxB.arc(midX, midY, 6371*scale, 0, Math.PI*2);
  ctxB.stroke();
